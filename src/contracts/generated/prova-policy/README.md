...
contract `prova-policy` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CDGMBYHO7A65PBCYTGAWL6EBFL5H2DTGCCPSTP5ABA4UFNEPNUWSF34S \
  --output-dir ./path/to/prova-policy
```
...
```json
"dependencies": {
  "prova-policy": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url https://soroban-testnet.stellar.org --network-passphrase \"Test SDF Network ; September 2015\" --id CDGMBYHO7A65PBCYTGAWL6EBFL5H2DTGCCPSTP5ABA4UFNEPNUWSF34S --name prova-policy"
}
```
...
```js
import { Contract, networks } from "prova-policy"
